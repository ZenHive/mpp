defmodule MPP.Method do
  ...
  callbacks that the `MPP.Plug` middleware uses during the 402 handshake:
    ...
    1. `method_name/0` — method identifier for protocol headers (grammar `[a-z][a-z0-9:_-]*`)
  ...
  **Intent = Schema, Method = Implementation.** Methods share the same intent
  ...
  Must be a stable identifier matching the reference-SDK grammar
  `[a-z][a-z0-9:_-]*` (mppx / mpp-rs #428): a lowercase ASCII letter followed by
  lowercase letters, digits, colons, underscores or hyphens. Any other shape is
  rejected by challenge parsing (`:invalid_method`) and by `MPP.Plug.init/1` at
  boot. The spec ABNF `payment-method-id = 1*LOWERALPHA` is stricter; both
  reference SDKs accept the looser grammar, so interop follows the SDKs.
  """
