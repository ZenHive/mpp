defmodule MPP.Intents.Session do
  ...
  def to_request(%__MODULE__{} = session) do
    ...
    |> put_optional("unitType", session.unit_type)
    |> put_optional("recipient", session.recipient)
    |> put_optional("suggestedDeposit", session.suggested_deposit)
    |> put_optional("externalId", session.external_id)
    |> put_optional("methodDetails", session.method_details)
  end
