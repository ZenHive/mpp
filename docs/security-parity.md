...
| `GHSA-8c63-r789-xrrf` | `CVE-2026-89420` (assigned by the EEF CNA 2026-09-15, publication pending) | HIGH | 0.16.2 | Session voucher that adds no new funds was served without a charge — unlimited units after one paid voucher (mpp-rs #415 parity) |
| `GHSA-p9fv-9w58-95x2` | `CVE-2026-87119` (assigned by the EEF CNA 2026-09-15, publication pending) | MEDIUM | 0.16.2 | Tempo subscription key authorization was not bound to the issuing challenge — captured activation credential replayable under a fresh challenge (mppx #882 parity) |
| `GHSA-8x7x-5j8g-8hcx` | `CVE-2026-88255` (assigned by the EEF CNA 2026-09-15, publication pending) | MEDIUM | 0.16.2 | Tempo pre-broadcast dedup reserve keyed on the caller-supplied encoding — a non-canonical re-encoding reserved a second slot (mppx #818 parity) |
| `GHSA-82qh-vrvm-gqvc` | `CVE-2026-89186` (assigned by the EEF CNA 2026-09-15, publication pending) | MEDIUM | 0.16.2 | `Payment-Receipt` and `Cache-Control: private` written before the downstream app ran — consumer `Cache-Control` could expose paid responses to shared caches (mpp-rs #381/#399 parity) |
| `GHSA-rp92-r4rc-42gw` (deleted) | — (deleted 2026-09-16 by GitHub Support at our request, so the id no longer resolves: the affected code landed after the 0.17.0 tag and first shipped in 0.17.1 together with its fix, so no released version was ever affected; the CNA review reached the same conclusion) | LOW | 0.17.1 | XRPL session redemption held the Destination lease through ledger validation with unbounded waiter queueing — a slow validation stalled every other redemption on that Destination |
| `GHSA-65c4-v2vw-rr64` | — (not requested: LOW-severity hardening; the CNA agreed on 2026-09-15 that it does not warrant a CVE) | LOW | 0.17.0 | Tempo canonical dedup reserve did not normalize one signature re-encoding — a second slot for one signed transaction; the live node rejected that encoding at broadcast (residual of `GHSA-8x7x-5j8g-8hcx`) |
| `GHSA-6xg7-3f46-g3g5` | — (to be requested from the EEF CNA) | HIGH | 0.19.0 | Solana push (`type="signature"`) credential was not bound to the issuing challenge — a third party could redeem an observed payment. Push is now opt-in and challenge-memo bound; every receipt path claims its signature once |
| `GHSA-2g9q-rg79-676x` | — (to be requested from the EEF CNA) | MEDIUM | 0.19.0 | Stripe subscription activation was deduplicated per challenge only — one payment method could activate the same plan again under a fresh challenge |
...
were requested on 2026-09-14 and assigned CVE ids on 2026-09-15 (publication pending on
the CNA side). The two LOW advisories from 0.17.0/0.17.1 were deliberately not requested;
on review the CNA confirmed neither warrants a CVE, and `GHSA-rp92-r4rc-42gw` was deleted by GitHub Support
on 2026-09-16 because the code it described never shipped in an affected release (see the
...
| mpp-rs #379 reject non-letter method identifiers (commit 7a6e517, 2026-08-10), superseded by mpp-rs #428 (commit 33a6bd7, 2026-09-21) aligning with mppx `[a-z][a-z0-9:_-]*` | Method-name confusion / smuggling via out-of-grammar identifiers | `Challenge.valid_method_name?/1` enforces the SDK grammar (lowercase letter first, then `[a-z0-9:_-]`) at parse time and at `MPP.Plug.init/1` — `challenge.ex`; the earlier `1*LOWERALPHA` tie-break to the spec was dropped on 2026-09-22 once both SDKs agreed (the spec ABNF is still stricter) |
...
| mpp-specs #362 (commit 7c23d8a, 2026-09-21) HMAC header slot vectors | Cross-implementation challenge-ID mismatch when `header` is advertised (spec text previously appended `header` after `opaque`) | Spec now matches the SDK layout we already used (`header` inserted before `opaque`); all three draft-01 test vectors pass byte-for-byte — `compute_id/2` in `challenge.ex`, pinned in `test/mpp/challenge_conformance_test.exs` (2026-09-22) |
| mpp-rs #428 (commit 33a6bd7, 2026-09-21) `\uXXXX` challenge auth-param escapes (mppx #813 parity) | Non-Latin-1 text in `WWW-Authenticate` breaking header construction / lossy round-trip | Already implemented from mppx #813: `escape_non_latin1/1` + `parse_quoted_string/2` decode incl. surrogate pairs — `headers.ex`. We reject a malformed `\u` escape or an unpaired surrogate (`:invalid_auth_params`) where both SDKs degrade to a literal / U+FFFD; stricter on malformed input only |
| mpp-rs #427 (commit b0628d3, 2026-09-21) validate challenge memo before fee-payer signing / broadcast | Funds settling on-chain before memo verification fails (client pays, receives no receipt) | `prepare_sponsored_transaction/5` runs `check_matched_memo_binding/3` before the dedup reservation, co-sign and broadcast — `tempo.ex` |
| mppx #904 (commit 1851f0f) / mpp-rs #421 (commit c29097a) challenge-bound charge memos mandatory | Attribution memo not bound to the challenge/realm accepted on the primary transfer | Default (no static memo) path already requires the binding on receipt logs, `type="transaction"` calls and machine-token routes (`transfer_memo_bound?/2`, `check_matched_memo_binding/3` — `tempo.ex`; Tasks 75 / 76). Removal of the residual static-`memo` opt-in is 📋 below |
...
| Static Tempo `memo` option removed upstream (mppx #904 / mpp-rs #421) — reference clients now ignore a server-advertised `memo`, so a static-memo route is unpayable by them | 📋 **Task 137** retires the option and aligns `MPP.Client.Providers.Tempo` |
...
**No open items** as of 2026-09-25.
